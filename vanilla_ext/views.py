...
from vanilla_ext.formsets import FormSetMixin, ModelFormSetMixin
from vanilla_ext.formsets import InlineFormSetFactory, GenericInlineFormSetFactory
...
class ModelFormSetView(ModelFormSetMixin, GenericModelView):
    ...
    def post(self, request):
        ...
        self.object_list = self.get_queryset()
        formset = self.get_formset(
            data=request.POST, files=request.FILES, queryset=self.object_list)
        if formset.is_valid():
            return self.formset_valid(formset)
        return self.formset_invalid(formset)
...
class InlineFormSetView(InlineFormSetFactory, GenericModelView):
    success_url = None
...
class GenericInlineFormSetView(GenericInlineFormSetFactory, InlineFormSetView):
    """
    Exactly the same as `InlineFormSetView` except that we override
    `InlineFormSetFactory` with `GenericInlineFormSetFactory`.
    """
    pass
